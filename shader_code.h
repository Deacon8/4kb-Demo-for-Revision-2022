...
const char *shader1_cs =
 ...
 "float s(vec3 v)"
 "{"
   "return fract(sin(dot(cross(v,vec3(33.,777.,66.)),vec3(7777.,3333.,7777.)))*77777.);"
 "}"
 "mat3 d(float v)"
 "{"
         ...
         "for(int n=-abs(a-X);n<=abs(a-X);n++)"
           "for(int g=-abs(a-n);g<=abs(a-n);g++)"
             "{"
               "if(ivec3(X,n,g)==S)"
                 "break;"
               "vec4 D=imageLoad(v,ivec3(S.x+X,S.y+n,0.));"
               "if(D.w-fract(m.w+1./e)<u)"
                 "Y+=1;"
             "}"
       "if(Y>y)"
         "m.w+=1./e,m=vec4(m.w),m=vec4(s(m.xyz),s(m.xyz+vec3(2.)),s(m.xyz+vec3(3.)),m.w);"
       "if(m.w>1.001)"
         "m=vec4(0.);"
     "}"
   "imageStore(v,S,m);"
